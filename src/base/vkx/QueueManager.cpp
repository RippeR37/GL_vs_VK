...
namespace vkx {
std::vector<vk::DeviceQueueCreateInfo> QueueManager::createInfos(const vk::PhysicalDevice& device)
{
...
QueueManager::QueueManager(const vk::PhysicalDevice& physicalDevice, const vk::Device& device)
    : _familyIndex(chooseFamilyIndex(physicalDevice))
...
uint32_t QueueManager::chooseFamilyIndex(const vk::PhysicalDevice& physicalDevice) const
{
    return createInfos(physicalDevice).front().queueFamilyIndex;
}
