                                                    ...
                                                    std::vector<VkQueueFamilyProperties>& queueFamilyProperties)
{
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        std::cout << "    VK_QUEUE_GRAPHICS_BIT:        "
                  << ((queueFamilyProperties[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) ? 1 : 0) << std::endl;
        std::cout << "    VK_QUEUE_COMPUTE_BIT:         "
                  << ((queueFamilyProperties[i].queueFlags & VK_QUEUE_COMPUTE_BIT) ? 1 : 0) << std::endl;
        std::cout << "    VK_QUEUE_TRANSFER_BIT:        "
                  << ((queueFamilyProperties[i].queueFlags & VK_QUEUE_TRANSFER_BIT) ? 1 : 0) << std::endl;
        std::cout << "    VK_QUEUE_SPARSE_BINDING_BIT:  "
                  << ((queueFamilyProperties[i].queueFlags & VK_QUEUE_SPARSE_BINDING_BIT) ? 1 : 0) << std::endl;
...
void SimpleBallsSceneTest::run()
{
    ...
    if (createVkInstance(vkInstance) != VK_SUCCESS) {
        ...
        throw std::runtime_error("Vulkan instance could not have been created!");
    }
    ...
    if (detectVkPhysicalDevices(vkInstance, vkPhysicalDevices) != VK_SUCCESS) {
        ...
        throw std::runtime_error("Vulkan physical device could not have been created!");
    }
    ...
    if (createVkDevice(vkPhysicalDevice, &vkDevice, queueFamilyIndex) != VK_SUCCESS) {
        ...
        throw std::runtime_error("Vulkan logic device could not have been created!");
    }
